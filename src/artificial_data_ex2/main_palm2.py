...
def send(prompt:str):
    response = str(genai.generate_text(prompt=prompt,temperature=0,top_p=1.0,max_output_tokens=40).result)
    return response
...
def main(args):
    ...
    generation_config = genai.GenerationConfig(temperature=0,top_p=1.0,max_output_tokens=40)
    ...
    with open(args.output_file, "w") as f, open(args.output_json_file, "w") as jf:
        for index in tqdm(range(len(inputs))):
            ...
            #response = gemini_pro.generate_content(prompt+inputs[index]+"\n".join(outputs[index][:args.depth]),generation_config=generation_config)
            #response = response.candidates[0].content.parts[0].text
            
            json_data = {
                "input": inputs[index],
                "output": outputs[index],
                "pred_output": str(response),
            }
